...
namespace nd {
    template <typename T>
    void ndarray_iterator<T>::advance() {
    ...
    template <typename T>
    ndarray_iterator<T>::ndarray_iterator(ndarray<T>* const x):
        m_iterable(x),
        m_index(x->ndim(), 0),
        m_offset(0) {}
    ...
    template <typename T>
    ndarray_iterator<T>::ndarray_iterator(ndarray_iterator<T> const& other):
    ...
    template <typename T>
    T& ndarray_iterator<T>::operator*() const {
    ...
    template <typename T>
    ndarray_iterator<T>& ndarray_iterator<T>::operator++() {
        advance();
        return *this;
    }
}
